...
- 输出里 `[ok]` = 已最新、`[skip]` = 拒绝、**`[pull]` 才是真的写入了文件**；
  加 `--dry-run` 时多一个 `[diff]`（内容不同、将备份后覆盖，但**没有真写**）。
...
Step 3: 根据查询结果采取行动：
  ...
  │                        （关键词补充写 initial_keyword_updates.csv，见 3.4.1.2）
  ├─ 找到 + category 为空 → 按 illion 标签补充分类，**同样写 reviews/<date>/initial_candidates.csv**
  └─ 未找到 → 写入 reviews/<date>/initial_candidates.csv（新增商户）
```

> ⚠️ **没有 `initial_category_updates.csv` 这条通道**：只有老脚本 `revise_candidates.py` 写过它，
> 而 `apply_rules.py` 与 `validate_candidates.py` 都只 glob `*_candidates.csv` —— 写了等于白写。
> 「商户已存在但 category 为空」与「新商户」合并走同一个文件（KB 加载是「同 keyword 后行胜出」，
> 追加行会覆盖旧的空 category）。
...
2. **商户已存在 + category 为空** → 根据 illion 标签补充分类，**也写入 `initial_candidates.csv`**（与第 3 条同一文件、同一条审批/落库链路；`initial_category_updates.csv` 是死文件，没有脚本读它）
...
keyword 必须是 `clean_text()` 归一化后的形式。⚠️ 匹配确实发生在 `clean_text()` 后的文本上，
但**加载侧只做 `str.strip()` + STOPWORDS 过滤 + 去重，不做 `clean_text()`**
（`initial_engine/domain/classification.py:228-233`，注释明写 "No apply(clean_text) needed"）。
所以格式不符合的 keyword 是**静默失配**，没有任何兜底。
...
规则：
  ...
  4. 竖线 | 分隔多个 keyword 时，| 两边不留空格（**格式约定**）
      ✅ 正确: "KEYWORD1|KEYWORD2|KEYWORD3"
      ❌ 避免: "KEYWORD1 | KEYWORD2 | KEYWORD3"
     ⚠️ 这**不是**引擎的硬要求 —— 加载侧确实会 strip：initial 引擎
     `classification.py:228` 的 `exploded["_kw_raw"].str.strip()`（注释明写
     "trailing spaces break the whole-word boundary check"），rent/gambling 的
     institution 层 `merchant_institution.py:122-123` 也逐变体 `.strip()`。
     之所以仍不留空格：现有 1.3M 个 keyword 无一例外都是无空格格式，
     保持一致便于比对；且**不该依赖**加载侧的容错
...
5. ❌ keyword 拼接时 | 两边加空格（如 `"KW1 | KW2"`）→ 统一 **无空格** 拼接（`"KW1|KW2"`）。
   注：加载侧其实会 strip（`classification.py:228`、`merchant_institution.py:123`），
   加空格**不会**真的失配 —— 但现有数据一律无空格，保持一致以免 `sync_rules.py` 报出无谓漂移
...
> `match_type` 列；`transfer_candidates.csv` 的 pattern 是 regex，同样没有 `match_type`）。
> 引擎语义集中在 `scripts/common.py` 的 `_ENGINE_TEXT` / `_MATCH_MODES` 两张表：
> `initial`/`rent`/`gambling`/`catch_all` 走 `clean_text()` + 全词 + `|` 变体拆分，
> `transfer` 走小写 + regex，`liability` 走大写 + `(?<![A-Za-z])` 边界，
> 其余保持原样（`dishonour`/`all_other_credit` 子串、`income`/`fee` regex）。
>
> **怀疑 gain 偏低时，拿 `hit_count` 对一遍**：逐条比对一致才说明模拟是对的。
> 2026-09-15 修复前 `initial`(329 命中) / `transfer`(1056) / `liability`(7) 三条
> 全部报 gain=0 —— 而 4.3 的风险分级拿 `gain > 0` 当初筛，等于把好规则判成「零增益」。
...
> ⚠️ **`gain == 0` 在下「零增益」结论前，先按 4.2 的说明核对 `hit_count`。**
> 这些候选是按「命中过高频未分类模式」筛出来的，真为 0 的情况很少。
...
**同时把 `initial_keyword_updates.csv` 的 status 一并改写** —— 该文件不叫 `*_candidates.csv`，
不在阶段五的写回范围内，而阶段八的 `apply_keyword_updates.py` 只处理 `status == confirmed` 的行
（`apply_keyword_updates.py:53`），漏改会直接打印 `[SKIP] No confirmed keyword updates` 整批跳过。
...
#### 5.5 导出 `confirmed_rules.json`（阶段六的输入，必做）

`test_rules.py` 的 `--rules` 是**必填**参数，只认一个 JSON 文件，**没有任何脚本会从
`*_candidates.csv` 自动生成它** —— 阶段六开跑前必须先把所有 `status == confirmed` 的行导出：

```python
# 合并 reviews/<date>/ 下所有 *_candidates.csv 里 status == confirmed 的行
import glob, json, sys
import pandas as pd

sys.path.insert(0, "scripts")
from common import candidate_pattern        # 与 baseline/test_rules 同源的取列逻辑

rules = []
for path in sorted(glob.glob("reviews/<date>/*_candidates.csv")):
    engine = path.replace("\\", "/").split("/")[-1].replace("_candidates.csv", "")
    df = pd.read_csv(path, encoding="utf-8-sig").fillna("")
    for _, row in df[df["status"].str.strip().str.lower() == "confirmed"].iterrows():
        # 各引擎候选 CSV 的列名不同，candidate_pattern 按 pattern → keywords → keyword 回退
        # （initial → keywords；transfer / liability → keyword；其余 → pattern）
        pattern = candidate_pattern(row)
        if not pattern:
            continue
        rules.append({
            "engine": engine,                                    # 必填：优先级 + 归一化 + 匹配方式都靠它
            "rule_name": str(row.get("rule_name") or row.get("merchant_name")
                             or row.get("counterparty") or pattern[:40]),
            "pattern": pattern,
            "match_type": str(row.get("match_type") or "keyword"),
            "category": str(row.get("category") or ""),
            "confidence": row.get("confidence") or None,
        })

json.dump(rules, open("reviews/<date>/confirmed_rules.json", "w", encoding="utf-8"),
          ensure_ascii=False, indent=2)
print(f"exported {len(rules)} confirmed rules")
```

**文件结构**（`test_rules.py` 的 `test_rules()` 开头，读入 `rules_data` 后按
`isinstance(rules_data, list)` 分派）：

- 顶层**数组** `[ {...}, {...} ]`（推荐），或
- 对象 `{"confirmed": [...], "edited": [...], "rejected": [...]}` —— `edited` 会**追加在
  `confirmed` 之后**（用编辑后的新值），`rejected` 被忽略

每条规则的字段：

| 字段 | 必填 | 默认 | 用途 |
|------|------|------|------|
| `pattern` | ✅ | — | 匹配模式；为空该条被跳过并计入 `skipped_rules` |
| `engine` | ⚠️ **必填** | `unknown` | 三处用途：`get_engine_priority()` 算优先级判「真冲突」、**选文本归一化**、**选匹配方式**。填成 `unknown` 会退化成「原始文本 + 子串匹配」 |
| `match_type` | 否 | `keyword` | 只是**覆盖项**：为 `regex` 时强制按正则处理；否则由 `match_mode()` 按引擎决定。`regex` 会先编译校验 |
| `rule_name` | 否 | `rule_{idx}` | 报告里的标识 |
| `category` | 否 | `""` | 冲突详情里的 `new_category` |
| `confidence` | 否 | `null` | 仅回显 |

> ✅ `test_rules.py` 与 `baseline.py diff` **共用** `common.match_series()` /
> `common.engine_texts()`：按各引擎自己的文本归一化 + 匹配语义打分
> （initial/rent/gambling/catch_all 走 `clean_text()` + `|` 变体全词；transfer 走小写正则；
> liability 走 `(?<![A-Za-z])` 边界；income/fee 走 `clean_text` 无关的 regex …）。
> 两者对同一批 30 条候选的结论一致，且与生成器写入的 `hit_count` **逐条相同**。
>
> ⚠️ **代价是必须如实填 `engine`**。引擎名写错或缺失 → 退化为「原始文本 + 子串匹配」→
> initial / transfer / liability / income 的增益**静默变 0**。实测同一批候选：
> 填对引擎 total_gain=1,553，旧的简化匹配只报 146（丢弃规则的门槛是 `gain > 10` /
> `gain > 0`，见阶段五/七 —— 报 0 的规则会被判「零增益」而卡在审批门）。
