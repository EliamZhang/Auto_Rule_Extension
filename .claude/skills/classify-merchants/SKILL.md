...
⚠️ **KB 的直接改动当前没有自动化推送通道**：`scripts/apply_rules.py --sync_to` 只处理
`reviews/<date>/*_candidates.csv` 里的确认规则（`--review_dir` 是必填参数，且 sync 只在
「写入了候选」的分支里发生），**不会同步你手工改过的 `merchant_kb.csv`**；`sync_rules.py`
则是只拉不推的。确认过后只能**人工把 `raw/initial_rule/merchant_kb.csv` 复制到
`<finv>/initial_engine/merchant_kb.csv`**（finv 侧 `initial_engine` 目录，非 `*_rule/`），
并由操作者自行确认落地结果。

## ⚠️ Gambling 已被人为清空

finv 于 2026-09-02 从 KB 中删除了全部 1,822 条 Gambling 商户，**当前 KB 的 Gambling 行数为 0**
