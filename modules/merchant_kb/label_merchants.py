...
DEFAULT_MERCHANT_KB = FINAL_OUTPUT
DEFAULT_CACHE = BASE_DIR / "cache" / "merchant_category_cache.json"
...
# KB 的列结构由 finv_category_V2 initial_engine 的加载契约决定
# （domain/classification.py:198 以 usecols 只读这 3 列），不可扩展。
# 2026-08-27 起 KB 重建为 3 列，以下列已废弃：
#   - link            → 仍保留在 MerchantClassification（落在 cache JSON）中作溯源
#   - category_source → 过去只会被写成常量 "AI"，无信息量
#   - *_updated_at    → 审计时间戳，可从 cache 推导
KB_FIELDNAMES = [
    "merchant_name",
    "keywords",
    "category",
...
def build_classification_cache_key(merchant_name: str, keywords: str, link: str = "") -> str:
    # ⚠️ 即使 KB 已无 link 列，这个空串分量也必须保留：删掉它会让 cache key 从
    # "name\nkeywords\n" 变成 "name\nkeywords"，导致现有 API 缓存整体失效。
    return "\n".join(
        (
...
def normalize_kb_row(row: dict[str, str]) -> dict[str, str]:
    # 只产出 KB_FIELDNAMES 的 3 列。多余的键会让 write_merchant_kb_rows 的
    # DictWriter（未设 extrasaction）直接抛 ValueError。
    return {
