...
DEFAULT_INPUT = BASE_DIR / "sample.csv"
DEFAULT_OUTPUT = BASE_DIR / "output" / "sample_verified.csv"
DEFAULT_CACHE = BASE_DIR / "cache" / "sample_verification_cache.json"
...
# 2026-08-27 起 KB 重建为 3 列，link / category_source / *_updated_at 已废弃。
KB_FIELDNAMES = [
    "merchant_name",
    "keywords",
    "category",
...
def normalize_kb_row(row: dict[str, str]) -> dict[str, str]:
    # 只产出 KB_FIELDNAMES 的 3 列。多余的键会让 save() 的 DictWriter
    #（未设 extrasaction）直接抛 ValueError。
    return {
    ...
    def append_ai_results(self, candidates: list[MerchantKBCandidate]) -> None:
        ...
        for candidate in candidates:
            ...
            for keyword_value, normalized_keyword in keywords_to_add:
                ...
                self._dirty = True
            if existing_keywords != split_kb_keywords(row["keywords"]):
