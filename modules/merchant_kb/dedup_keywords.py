...
DEFAULT_REPORT_PATH = BASE_DIR / "output" / "clean_report.csv"
...
def has_column(path: Path, column: str) -> bool:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Only clean rows with keyword_updated_at >= this timestamp. "
             "Requires an input file that still has that column.",
    )
    parser.add_argument(
        "--full",
        action="store_true",
